...
class LocalStorageDraftService {
  ...
  private readonly DEFAULT_STORAGE_LIMIT = 5 * 1024 * 1024; // 5MB
  private readonly STORAGE_THRESHOLD = 0.8; // 80%
  private readonly DEFAULT_DRAFT_LIMIT = 10;
    ...
    try {
      ...
      if (this.isStorageNearLimit()) {
        ...
        try {
          ...
          throw new Error("Storage full - unable to save draft");
        }
      } else {
  ...
  getStorageInfo(): StorageInfo {
    ...
    try {
      ...
      const used = new Blob([draftData]).size;
      
      return {
        used,
        total: this.DEFAULT_STORAGE_LIMIT,
  ...
  isStorageNearLimit(): boolean {
    ...
    return info.used > (info.total * this.STORAGE_THRESHOLD);
  }

  cleanupOldDrafts(keepCount: number = this.DEFAULT_DRAFT_LIMIT): void {
    if (!this.isDraftSupported()) return;
